(() => {
  ...
  document.addEventListener("click", event => {
    ...
    if (!button) return;
    const context = [button.textContent, button.value, button.getAttribute("aria-label"), button.closest("form")?.innerText?.slice(0, 500)].filter(Boolean).join(" ");
    if (!POSITIVE.test(context) || NEGATIVE.test(context)) return;
  ...
  function showReminder(site, cooldownHours) {
    ...
    shadow.innerHTML = `
      ...
      <aside class="card" role="status" aria-label="Previously used accounts">
        <div class="top"><span class="mark">i</span><span class="title">Previously used here</span><button class="close" aria-label="Dismiss">×</button></div>
        <div class="label">${accounts.length === 1 ? "Account" : "Accounts"}</div>
        ${accounts.map(a => `<div class="account"><div class="email"></div><div class="time">Last used ${relativeTime(a.lastUsedAt)}</div></div>`).join("")}
        ${site.accounts.length > 3 ? `<div class="more">+${site.accounts.length - 3} more in Identity Recall</div>` : ""}
      </aside>`;
    [...shadow.querySelectorAll(".email")].forEach((node, index) => { node.textContent = accounts[index].email; });
